 ...
 */
async function apiRequest(endpoint: string, method: 'GET' | 'POST', body?: any, errorMessage: string = 'Request failed') {
    const config: any = {
        method,
...
export const ArcAppKitAdapter = {
    ...
    async swapTokens(amount: number, fromToken: string, toToken: string, tokenAddress: string) {
        ...
        return apiRequest('/api/swap/execute', 'POST', {
     ...
     */
    async sendUnifiedBalance(amount: number, destinationAddress: string, memo?: string) {
        const { registeredUser } = useStore.getState();
        if (!registeredUser?.supabaseUid) throw new Error("User not registered");

        console.log(`[Adapter] Sending unified balance: ${amount} to ${destinationAddress}${memo ? ` with memo: ${memo}` : ''}`);

        return apiRequest('/api/transfer/execute', 'POST', {
            userId: registeredUser.supabaseUid,
            amount,
            destinationAddress,
            memo
    ...
    async bridgeTokenCCTP(amount: number, destinationAddress: string, destinationDomain: number) {
        ...
        return apiRequest('/api/bridge/cctp', 'POST', {
            userId: registeredUser.supabaseUid,
            amount,
            destinationAddress,
            destinationDomain
    ...
    async bridgeToken(amount: number, fromNetwork: string, toNetwork: string) {
        ...
        return apiRequest('/api/bridge/execute', 'POST', {
            userId: registeredUser.supabaseUid,
            amount,
            fromNetwork,
            toNetwork
    ...
    async executeWithdraw(amount: number, bank: string) {
        ...
        return apiRequest('/api/withdraw/execute', 'POST', {
            userId: registeredUser.supabaseUid,
            amount,
            bank
        }, 'Withdraw failed');
    ...
    async executePurchase(amount: number, product: string) {
        ...
        return apiRequest('/api/purchase/execute', 'POST', {
            userId: registeredUser.supabaseUid,
            amount,
            product
        }, 'Purchase failed');
    ...
    async getBalance() {
        ...
        return apiRequest(`/api/balance/${registeredUser.supabaseUid}`, 'GET', undefined, 'Failed to fetch balance');
    },
    ...
    async getLiveRate(fromToken: string, toToken: string) {
        return apiRequest(`/api/rates?from=${fromToken}&to=${toToken}`, 'GET', undefined, 'Failed to fetch rate');
    },
     ...
     */
    async getTokens() {
        return apiRequest(`/api/tokens`, 'GET', undefined, 'Failed to fetch tokens');
    },
