    ...
    <AnimatePresence mode="wait">
      ...
      {viewState === "ecommerce" && (
        platformConfig && platformConfig.ecommerceEnabled === false ? (
          renderLockedScreen("Modul Marketplace E-Commerce", "Fitur Marketplace dan pembelian produk dinonaktifkan sementara oleh Administrator Sistem.")
        ) : (
      ...
      {viewState === "merchant" && (
        platformConfig && platformConfig.merchantEnabled === false ? (
          renderLockedScreen("Modul Kasir F&B Merchant", "Fitur kasir digital merchant partner sedang dinonaktifkan sementara untuk pembatasan wilayah.")
        ) : (
      ...
      {viewState === "faucet" && (
        platformConfig && platformConfig.faucetEnabled === false ? (
          renderLockedScreen("Klaim Faucet Arc Testnet", "Sinyal pembagian token gas gratis sedang ditutup sementara oleh Penjaga Devnet.")
        ) : (
      ...
      {viewState === "bayarVA" && (
        platformConfig && platformConfig.vaEnabled === false ? (
          renderLockedScreen("Kode Bayar Virtual Account (VA)", "Layanan pembayaran tagihan VA dihentikan sementara oleh Admin.")
        ) : (
      ...
      {viewState === "swap" && (
        platformConfig && platformConfig.swapEnabled === false ? (
          renderLockedScreen("Fitur Swap Pool USDC", "Layanan pertukaran aset dibekukan sementara oleh Admin untuk penyeimbangan likuiditas.")
        ) : (
      ...
      {viewState === "stablestake" && (
        platformConfig && platformConfig.stableStakeEnabled === false ? (
          renderLockedScreen("Stablestake Deposit Yield", "Staking pool saat ini dikunci untuk konsolidasi bunga native-USDC.")
        ) : (
      ...
      {viewState === "receiveVA" && (
        platformConfig && platformConfig.vaEnabled === false ? (
          renderLockedScreen("Virtual Account (VA)", "Setoran deposit via Virtual Account Bank saat ini sedang tidak beroperasi.")
        ) : (
      ...
      {viewState === "receiveQRIS" && (
        platformConfig && platformConfig.qrisEnabled === false ? (
          renderLockedScreen("Deposit QRIS Standar", "Gerbang pembuatan kode bayar QRIS dinonaktifkan sementara oleh pemroses pembayaran.")
        ) : (
          <ReceiveQRISScreen
      ...
      {viewState === "arcbird" && (
        platformConfig && platformConfig.arcBirdEnabled === false ? (
          renderLockedScreen("Permainan Arc Bird", "Game Mini-App Arc Bird saat ini ditangguhkan untuk pembaharuan leaderboard musiman.")
        ) : (
      ...
      {viewState === "connectEWallet" && (
        platformConfig && platformConfig.eWalletConnectionEnabled === false ? (
          renderLockedScreen("Integrasi e-Wallet", "Pintu gerbang penarikan saldo e-Wallet lokal dialihkan sementara oleh Admin.")
        ) : (
      ...
      {viewState === "scanQR" && (
        platformConfig && platformConfig.scanQrEnabled === false ? (
          renderLockedScreen("Pemindai Code QR", "Sensor pemindaian kamera backend dinonaktifkan sementara untuk peningkatan lisensi.")
        ) : (
          <ScanQRScreen
            onBack={() => setViewState("home")}
      ...
      {viewState === "aiAgent" && (
        platformConfig && platformConfig.aiAgentEnabled === false ? (
          renderLockedScreen("AI Agent Co-Pilot", "AI Asisten Keuangan dinonaktifkan sementara oleh admin demi penyesuaian regulasi kognitif.")
        ) : (
      ...
      {viewState === "transfer" && (
        platformConfig && platformConfig.transferEnabled === false ? (
          renderLockedScreen("Kirim Saldo & Transfer", "Transfer Peer-to-Peer ditutup sementara karena adanya pemeliharaan jaringan.")
        ) : (
          <TransferScreen
            onBack={() => setViewState("home")}
      ...
      {viewState === "batchTransfer" && (
        platformConfig && platformConfig.batchTransferEnabled === false ? (
          renderLockedScreen("Transfer Massal (Batch)", "Multi-transfer instan dinonaktifkan sementara untuk pencegahan penimbunan gas.")
        ) : (
          <BatchTransferScreen
            onBack={() => setViewState("transfer")}
      ...
      {viewState === "withdraw" && (
        platformConfig && platformConfig.withdrawEnabled === false ? (
          renderLockedScreen("Tarik Saldo (Withdraw)", "Penarikan USDC keluar ekosistem ditangguhkan sementara demi keamanan audit kas.")
        ) : (
          <WithdrawScreen 
            onBack={() => setViewState("depositOptions")}
      ...
      {viewState === "bridge" && (
        platformConfig && platformConfig.bridgeEnabled === false ? (
          renderLockedScreen("CCTP Bridge Lintas Rantai", "Jembatan pembakaran & pencetakan USDC Circle sedang dinonaktifkan sementara.")
        ) : (
          <BridgeScreen 
            onBack={() => setViewState("home")}
            onSuccess={() => setViewState("home")}
      ...
      {viewState === "adminDashboard" && (
        <AdminDashboardScreen onBack={() => setViewState("home")} onNavigate={setViewState} />
      )}
