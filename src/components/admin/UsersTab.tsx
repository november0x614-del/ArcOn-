          ...
          <table className="w-full text-left border-separate border-spacing-0">
            <thead>
              <tr className="bg-slate-50/80 sticky top-0 z-10">
                <th className="px-6 py-4 text-[10px] font-extrabold text-slate-400 uppercase tracking-widest border-b border-slate-100">
                  Identity
                </th>
                <th className="px-6 py-4 text-[10px] font-extrabold text-slate-400 uppercase tracking-widest border-b border-slate-100 hidden md:table-cell">
                  Wallet Identifier
                </th>
                <th className="px-6 py-4 text-[10px] font-extrabold text-slate-400 uppercase tracking-widest border-b border-slate-100 text-center">
                  Status
                </th>
                <th className="px-6 py-4 text-[10px] font-extrabold text-slate-400 uppercase tracking-widest border-b border-slate-100 text-right">
                  Action
                </th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-50">
              {loading && users.length === 0 ? (
                <tr>
                  <td
                    colSpan={4}
                    className="px-6 py-12 text-center text-slate-400 text-[13px] font-bold animate-pulse"
                  >
                ...
                <tr>
                  <td
                    colSpan={4}
                    className="px-6 py-12 text-center text-slate-400 text-[13px] font-bold"
                  >
                ...
                filteredUsers.map((u, i) => (
                  ...
                  >
                    <td className="px-6 py-3.5">
                      <div className="flex items-center gap-3">
                        ...
                        <div className="min-w-0">
                          <div className="font-bold text-slate-800 text-[13px] tracking-tight truncate max-w-[140px] md:max-w-none">
                            {u.name}
                          </div>
                          <div className="text-[10px] text-slate-400 font-medium truncate max-w-[140px] md:max-w-none">
                            {u.email}
                          </div>
                        </div>
                      </div>
                    ...
                    <td className="px-6 py-3.5 hidden md:table-cell">
                      <div className="flex flex-col">
                        <span className="text-[11px] font-mono text-slate-500 font-bold tracking-tighter">
                          {u.wallet.substring(0, 12)}...
                          {u.wallet.substring(u.wallet.length - 8)}
                        </span>
                    ...
                    <td className="px-6 py-3.5 text-center">
                      <span
                        className={`px-2.5 py-0.5 rounded-full text-[9px] font-extrabold uppercase tracking-tight ${
                          u.status === "Blocked"
