...
export const WalletCard = React.memo(function WalletCard({
  ...
  return (
    ...
    >
      ...
      <div className="flex justify-between items-center z-10 relative gap-3">
        <div className="flex flex-col text-left flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            <span
              className={`text-[12px] sm:text-[13px] font-medium ${styles.subText} whitespace-nowrap`}
            >
              Est total value
            </span>
            {showBalance ? (
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <span
              className={`text-[11px] sm:text-[12px] ${styles.subText} border-b border-dashed ${styles.pnlBorder} pb-0.5 whitespace-nowrap`}
            >
              PnL
            </span>
      ...
      {/* Ambient Glow */}
      <div
        className={`absolute top-0 right-0 w-32 h-32 ${styles.glow} blur-[60px] rounded-full -translate-y-12 translate-x-12 pointer-events-none`}
      ></div>
    </div>
  );
});
