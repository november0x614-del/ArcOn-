...
export function UIDCard({ userName, isBlurred = false }: UIDCardProps) {
  ...
  const rawUid = (registeredUser?.supabaseUid || "8f7e-ffa1-0fb0-c52a")
    .replace(/-/g, "")
    .toUpperCase();
  const formattedUid =
    rawUid
  ...
  return (
    <div style={{ perspective: 1200 }} className="w-full max-w-[500px] mx-auto">
      ...
      >
        {/* Accent Graphic */}
        <div
          className={`absolute top-[-10%] right-[-10%] w-[50%] h-[120%] border-[12px] ${styles.accentBorder} rounded-[100px] pointer-events-none rotate-12`}
        ></div>
        <div
          className={`absolute top-[10%] right-[-5%] w-[40%] h-[40%] border-[2px] ${styles.accentBorder} rounded-full pointer-events-none opacity-50`}
        ></div>
        ...
        >
          <div className="flex items-center gap-2">
            <div
              className={`w-8 h-8 rounded-lg ${currentTheme === "default" ? "bg-slate-800" : currentTheme === "premium" ? "bg-[#d4c085]" : "bg-emerald-500"} flex items-center justify-center shadow-lg`}
            >
          ...
          <div
            className={`w-8 h-8 rounded-lg ${currentTheme === "default" ? "bg-slate-50" : "bg-white/50"} border ${styles.borderClass} flex items-center justify-center`}
          >
            <div className="grid grid-cols-2 gap-[1px]">
              <div
                className={`w-[3px] h-[3px] ${currentTheme === "default" ? "bg-slate-900" : currentTheme === "premium" ? "bg-slate-800" : "bg-emerald-900"} rounded-[1px]`}
              ></div>
              <div
                className={`w-[3px] h-[3px] ${styles.uidSubText} rounded-[1px]`}
              ></div>
              <div
                className={`w-[3px] h-[3px] ${styles.uidSubText} rounded-[1px]`}
              ></div>
        ...
        >
          <div className="flex flex-col items-start">
            <span
              className={`${styles.uidText} font-bold text-[14px] uppercase tracking-wider`}
            >
              {userName}
            </span>
          </div>
