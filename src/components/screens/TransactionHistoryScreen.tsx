        ...
        {!selectedTx ? (
          ...
          <div className="p-4 animate-in slide-in-from-right duration-300 h-full flex flex-col">
            ...
            <div className="bg-white rounded-3xl p-6 shadow-sm border border-slate-100 flex flex-col items-center text-center">
              <div
                className={`w-16 h-16 rounded-full border-[3px] flex items-center justify-center mb-4 ${getTxBg(selectedTx.type)}`}
              >
                {getTxIcon(selectedTx.type)}
              </div>
              <h2 className="text-[18px] font-bold text-slate-800 mb-1">
                {selectedTx.title}
              </h2>
              <div
                className={`text-[32px] font-extrabold font-mono tracking-tight my-2 ${selectedTx.amount.startsWith("+") ? "text-emerald-500" : "text-slate-800"}`}
              >
              ...
              <div className="w-full flex justify-between items-center text-[14px] mb-4">
                ...
                <span className="font-mono font-medium text-slate-800 bg-slate-50 px-2 py-1 rounded">
                  TRX-{Math.floor(Math.random() * 9000000) + 1000000}
                </span>
              </div>
              <div className="w-full flex justify-between items-center text-[14px] mb-4">
              ...
              <div className="w-full flex justify-between items-center text-[14px] mb-4">
                ...
                <span className="font-medium text-slate-800 font-mono">
                  {selectedTx.metadata?.fee ||
                    (selectedTx.type === "deposit" ? "0.0000" : "0.0001")}{" "}
                  USDC
                </span>
              </div>
              {selectedTx.txHash && (
                <div className="w-full bg-slate-100 border border-slate-200 rounded-xl p-4 mt-2 flex flex-col gap-2">
                  <div className="flex items-center justify-between">
                    ...
                    <div
                      className={`w-2 h-2 rounded-full ${selectedTx.status === "pending" ? "bg-amber-500 animate-pulse" : "bg-emerald-500"}`}
                    ></div>
                  </div>
                  ...
                  {selectedTx.txHash.startsWith("0x") ? (
                    <a
                      href={
                        selectedTx.txHash
                          ? getExplorerUrl(selectedTx.txHash)
                          : selectedTx.explorerUrl || "#"
                      }
                      target="_blank"
                      rel="noopener noreferrer"
                      className="mt-2 w-full bg-white border border-blue-200 text-slate-800 font-bold py-2 rounded-lg text-[13px] flex items-center justify-center gap-1 hover:bg-slate-200 active:scale-95 transition-all"
                    >
                      View on Arc Explorer <ExternalLink size={14} />
                    </a>
                  ) : (
                    <div className="mt-2 text-[10px] text-slate-500 font-medium">Pending Blockchain Confirmation</div>
                  )}
                </div>
              )}
            </div>
