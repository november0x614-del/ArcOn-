...
export function PasswordScreen({ onBack, email, onLogin, onForgotPassword, isLoading }: PasswordScreenProps) {
  ...
  return (
    <div className="w-full h-full bg-white relative flex flex-col justify-between overflow-hidden">
      ...
      <div className="relative h-[45%] bg-gradient-to-b from-slate-800 to-slate-900 rounded-b-[40px] flex flex-col justify-center items-center px-6 overflow-hidden shrink-0 shadow-lg">
        ...
        <h1 className="text-2xl font-bold text-slate-800 tracking-tight mb-1 text-center drop-shadow-sm">Welcome Back</h1>
      ...
      <div className="flex-1 px-8 pt-8 pb-10 flex flex-col bg-white">
        ...
        <div className="flex-1 flex flex-col justify-center">
            <h2 className="text-[16px] font-bold text-slate-800 mb-6 text-center">Authentication</h2>
            ...
            >
                {showPassword ? <Eye size={22} strokeWidth={2} /> : <EyeOff size={22} strokeWidth={2} />}
            </button>
            </div>

             <button 
            onClick={() => password.length > 0 && !isLoading && onLogin(email, password)}
