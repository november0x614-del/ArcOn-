...
import { ArrowLeft, Wallet, ExternalLink, ShieldCheck, Zap } from 'lucide-react';
import { useArcWallet } from '../../lib/useArcWallet';

const EWALLETS = [
  { id: 'gopay', name: 'GoPay', iconColor: 'text-green-500', isConnected: true, balance: 'Rp 125.000', type: 'e-wallet' },
  { id: 'dana', name: 'DANA', iconColor: 'text-blue-500', isConnected: false, type: 'e-wallet' },
  { id: 'shopeepay', name: 'ShopeePay', iconColor: 'text-orange-500', isConnected: false, type: 'e-wallet' },
];

export function ConnectEWalletScreen({ onBack }: { onBack: () => void }) {
  const [wallets, setWallets] = useState(EWALLETS);
  const { status, address, isBusy, connect, disconnect, error } = useArcWallet();
  ...
  return (
    <div className="w-full h-full bg-[#f6f8fb] relative flex flex-col z-50 animate-in slide-in-from-right duration-300">
      ...
      <div className="flex items-center px-4 py-4 shrink-0 bg-slate-900 shadow-md z-10">
        <button onClick={onBack} className="p-2 hover:bg-white/10 rounded-full transition-colors border-0 bg-transparent">
          <ArrowLeft size={20} className="text-white" />
        </button>
        <h2 className="font-bold text-[16px] text-white ml-2 uppercase tracking-wide">Connect Asset</h2>
      </div>

      <div className="flex-1 overflow-y-auto px-4 py-6 pb-24 scrollbar-hide">
        {/* Web3 Wallet Section */}
        <div className="mb-8">
           <h3 className="text-[11px] font-bold text-slate-400 uppercase tracking-widest mb-3 px-1">Web3 Connection</h3>
           <div className="bg-slate-900 rounded-[24px] p-5 shadow-xl border border-white/10 relative overflow-hidden group">
              ...
              <div className="flex items-start justify-between mb-4 relative z-10">
                 <div className="flex items-center gap-3">
                    ...
                    <div>
                       <h4 className="text-white font-bold text-[17px] tracking-tight">Arc Wallet</h4>
                       <p className="text-slate-400 text-[11px]">Connect via MetaMask or Rabby</p>
                    </div>
                 </div>
                 {status === 'connected' && (
              ...
              {status === 'connected' ? (
                <div className="space-y-4">
                   <div className="bg-white/5 border border-white/10 rounded-xl p-3">
                      <p className="text-[10px] text-slate-500 font-bold uppercase mb-1">Connected Address</p>
                      <p className="text-[13px] text-slate-200 font-mono break-all">{address}</p>
                   </div>
                   <button 
                     onClick={disconnect}
                     disabled={isBusy}
                ...
                <div className="space-y-4">
                   <p className="text-slate-400 text-[12px] leading-relaxed">
                      Connect your self-custody wallet to interact directly with the Arc Network and manage your USDC.
                   </p>
                   <button 
                     onClick={connect}
                   ...
                   {status === 'not-installed' && (
                     <p className="text-amber-500 text-[10px] text-center font-medium">
                        Wallet extension not detected. <a href="https://metamask.io" target="_blank" rel="noreferrer" className="underline">Install MetaMask</a>
                     </p>
                   )}
        ...
        {/* E-Wallet Section */}
        <div>
          <h3 className="text-[11px] font-bold text-slate-400 uppercase tracking-widest mb-3 px-1">E-Wallet Assets</h3>
          <div className="flex flex-col gap-3">
            {wallets.map(wallet => (
              <div key={wallet.id} className="bg-white rounded-[20px] p-4 shadow-sm flex items-center justify-between border border-slate-100 hover:shadow-md transition-all">
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex flex-col">
                    ...
                    {wallet.isConnected ? (
                      <span className="text-[12px] font-bold text-[#008fcd] mt-0.5">{wallet.balance}</span>
                    ) : (
                      <span className="text-[11px] font-medium text-slate-400 mt-0.5">Click to link account</span>
                    )}
                  </div>
                </div>
