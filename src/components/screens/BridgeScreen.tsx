...
export function BridgeScreen({ onBack, onSuccess }: BridgeScreenProps) {
  ...
  const handleBridge = async () => {
    ...
    if (mode === 'outbound' && numAmount > balance) {
    ...
    try {
      if (mode === 'outbound') {
        ...
        console.log(`Simulating inbound bridge from ${fromNetwork.name} to Arc for ${numAmount} USDC`);
      ...
      <div className="absolute inset-0 bg-[#f8fafc] z-[70] flex flex-col p-6 animate-in fade-in duration-300">
         <div className="flex-1 flex flex-col items-center justify-center pt-10">
            ...
            <p className="text-slate-500 text-[14px] text-center max-w-[280px] mb-12">
               Your bridge is in progress. This typically takes 1-3 minutes depending on Circle CCTP attestation.
            </p>

            <div className="w-full max-w-sm bg-white p-6 rounded-[28px] shadow-sm border border-slate-100 flex flex-col gap-6">
               {/* Stepper */}
               {[
                 { id: 'broadcasting', label: 'Burn on Arc Native', desc: '1-confirmation finality burn' },
                 { id: 'attesting', label: 'Circle Attestation', desc: 'Validating CCTP transaction' },
                 { id: 'claiming', label: `Mint to ${toNetwork.name}`, desc: 'Executing receiveMessage' }
               ].map((p, idx) => {
                 ...
                 return (
                   <div key={p.id} className="flex gap-4 relative">
                     {idx !== 2 && (
                       <div className={`absolute left-4 top-8 bottom-[-24px] w-[2px] rounded-full transition-colors ${isDone ? 'bg-slate-800' : 'bg-slate-100'}`} />
                     )}
                     <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 z-10 transition-colors ${isDone ? 'bg-slate-800 text-white' : isActive ? 'bg-white border-2 border-slate-800 text-slate-800' : 'bg-slate-100 text-slate-400'}`}>
                        {isDone ? <CheckCircle2 size={16} /> : (isActive ? <Loader2 size={14} className="animate-spin" /> : <span className="font-bold text-[12px]">{idx + 1}</span>)}
                     </div>
                     <div className={`flex flex-col ${isPending ? 'opacity-40' : ''}`}>
         ...
         <div className="mt-auto px-4 pb-6 text-center">
            <p className="text-[11px] font-bold text-slate-400 mb-2">You can safely close this screen</p>
            <button 
              onClick={() => { setStep('form'); setShowPending(true); }}
      ...
      <div className="absolute bottom-0 left-0 right-0 p-6 bg-white border-t border-slate-100 z-20 shadow-2xl">
        <button 
          ...
          className={`w-full bg-slate-900 text-white py-4.5 rounded-full font-black text-[16px] shadow-lg hover:bg-slate-800 transition-all active:scale-[0.98] border-0 cursor-pointer flex items-center justify-center gap-2 ${
            (!amount || parseFloat(amount) <= 0 || parseFloat(amount) > balance || !destinationAddress) ? 'opacity-50 cursor-not-allowed' : ''
          }`}
        >
          Initiate Bridge
        </button>
      ...
      {showPending && (
         <div className="absolute inset-0 z-[90] bg-white flex flex-col animate-in slide-in-from-bottom duration-300">
            <div className="flex justify-between items-center px-5 py-6 border-b border-slate-100">
               <h3 className="font-black text-[18px] text-slate-900">Pending Bridges</h3>
               <button onClick={() => setShowPending(false)} className="p-2 bg-slate-50 rounded-full border-0 cursor-pointer">
            ...
            <div className="p-5 flex-1 overflow-y-auto bg-[#f8fafc]">
               <div className="bg-white rounded-2xl p-5 border border-slate-200 shadow-sm mb-4">
                  <div className="flex justify-between items-center mb-3">
                     <span className="text-[12px] font-bold text-slate-400">Tx: 0x93dc...82f1</span>
                     <span className="text-[10px] font-bold bg-amber-100 text-amber-600 px-2 py-1 rounded-md">Attesting</span>
                  </div>
                  <div className="flex items-center gap-3 mb-4">
                     <span className="font-black text-[20px] text-slate-900">10.00 USDC</span>
                     <ChevronRight size={14} className="text-slate-300" />
                     <div className="flex items-center gap-1.5">
                        <div className="w-5 h-5 bg-indigo-500 rounded flex items-center justify-center text-white"><Globe size={10} /></div>
                        <span className="text-[14px] font-bold text-slate-700">Ethereum</span>
                     </div>
                  </div>
