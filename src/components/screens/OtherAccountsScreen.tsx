            ...
            <div key={i} className="bg-white p-5 rounded-3xl shadow-sm border border-slate-200/60 hover:shadow-md transition-shadow cursor-pointer group active:scale-[0.98]">
              <div className="flex justify-between items-start mb-4">
                <div className="flex items-center gap-3">
                  <div className={`p-2.5 rounded-2xl ${acc.isWeb3 ? 'bg-slate-100 text-slate-800' : 'bg-slate-100 text-slate-500'}`}>
                    <Wallet size={20} />
                  </div>
                  <div>
                ...
                {acc.isWeb3 && (
                   <span className="text-[8px] font-black text-slate-800 bg-slate-100 px-1.5 py-0.5 rounded border border-slate-200 tracking-widest uppercase">Web3</span>
                )}
              </div>
