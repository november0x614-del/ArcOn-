                ...
                {activeTabs[activeRekeningTab]?.name === "My Wallet" && transactions.filter(t => t.type === 'payment').length > 0 && (
                   ...
                   >
                     <div className="flex justify-between items-center px-1">
                        <h3 className="text-[14px] font-bold text-slate-800">Recent Marketplace Orders</h3>
                        <button onClick={() => onNavigate("transactionHistory")} className="text-[11px] font-bold text-blue-600">See All</button>
                     </div>
                     <div className="bg-white rounded-2xl p-4 border border-slate-100 space-y-3">
                        {transactions.filter(t => t.type === 'payment').slice(0, 2).map((tx) => (
                           <div key={tx.id} className="flex justify-between items-center py-1 border-b border-slate-50 last:border-0 pb-3 last:pb-0">
                              <div className="flex items-center gap-3">
                                 <div className="w-9 h-9 bg-slate-100 rounded-lg flex items-center justify-center text-slate-800">
                                    <ShoppingBag size={18} />
                                 </div>
                                 <div className="text-left">
                                    <p className="text-[13px] font-bold text-slate-900">{tx.metadata?.recipientName || "Order"}</p>
                                    <p className="text-[10px] text-slate-500">{tx.timestamp}</p>
                                 </div>
                              </div>
                              <span className="text-[13px] font-mono font-black text-slate-900">{tx.amount} USDC</span>
                           </div>
                        ))}
                     </div>
                   </motion.div>
