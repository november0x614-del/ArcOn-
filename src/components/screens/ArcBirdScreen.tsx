      ...
      <div className="flex-1 overflow-y-auto p-6 flex flex-col gap-6">
        ...
        <div className="grid grid-cols-2 gap-3 w-full">
          <div className="bg-purple-50/50 border border-purple-100 rounded-xl p-3">
            <span className="text-[10px] uppercase font-bold text-purple-400 block mb-0.5">Your Highscore</span>
            <div className="flex items-center justify-center gap-1">
              <Trophy className="text-amber-500" size={14} />
              <span className="text-[18px] font-black text-purple-900 font-mono">{highScore} ARC</span>
            </div>
          </div>
          <div className="bg-amber-50/50 border border-amber-100 rounded-xl p-3">
            <span className="text-[10px] uppercase font-bold text-amber-500 block mb-0.5">Time Left</span>
            <span className="text-[18px] font-black text-amber-700 font-mono">{gameTimeLeft}s</span>
          </div>
        </div>
        ...
        <div className="w-full h-[240px] bg-indigo-950 rounded-[28px] border-4 border-indigo-900 relative overflow-hidden shadow-2xl flex flex-col justify-between p-4 select-none">
          ...
          <div className="relative z-10 flex justify-between items-center w-full">
            <span className="text-[10px] font-mono text-purple-300 uppercase tracking-widest bg-indigo-900/80 px-2 py-0.5 rounded border border-purple-500/30">Arc Arcade System</span>
            <div className="flex items-center gap-1.5">
              <span className="w-2 h-2 rounded-full bg-red-500 animate-pulse"></span>
              <span className="text-[14px] font-bold text-white font-mono">{gameScore} pts</span>
            </div>
          </div>

          <div className="relative z-10 flex justify-center items-center h-full">
            <motion.div
              animate={isGameActive ? { y: [0, -32, 0], rotate: [0, -10, 15, 0] } : { y: 0 }}
          ...
          {!isGameActive && (
            <div className="absolute inset-0 bg-slate-950/70 z-20 flex flex-col items-center justify-center gap-3 font-sans animate-in zoom-in-95 duration-200">
              ...
              <p className="text-[15px] font-bold text-white uppercase tracking-wider">Ready to Play?</p>
              <button 
                onClick={() => {
                  setGameScore(0);
                  setGameTimeLeft(15);
        ...
        {isGameActive && (
          <button
            onClick={() => {
              ...
              setGameScore((s) => s + 5);
              setFloatingTexts((prev) => [...prev, { id: Date.now(), x: rx, y: ry, text: "+5 ARC!" }]);
              setTimeout(() => {
