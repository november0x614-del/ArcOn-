    ...
    <div className="w-full h-full bg-slate-50 relative flex flex-col z-50 animate-in slide-in-from-right duration-300">
       ...
       <div className="flex-1 overflow-y-auto p-5 flex flex-col gap-4">
          <div className="text-center my-4">
            <span className="bg-slate-200/50 text-slate-500 font-semibold px-3 py-1 rounded-full text-[10px] uppercase tracking-widest">Hari Ini</span>
          </div>
          
          {messages.map((msg) => (
            <div key={msg.id} className={`flex ${msg.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
              <div className={`max-w-[75%] p-3.5 rounded-2xl ${msg.sender === 'user' ? 'bg-[#005faa] text-white rounded-tr-sm' : 'bg-white text-slate-700 shadow-sm border border-slate-100 rounded-tl-sm'}`}>
                 <p className="text-[13px] leading-relaxed">{msg.text}</p>
                 <span className={`text-[10px] block mt-1.5 text-right opacity-70 ${msg.sender === 'user' ? 'text-blue-100' : 'text-slate-400'}`}>{msg.time}</span>
              </div>
            </div>
          ))}
          {isLoading && (
