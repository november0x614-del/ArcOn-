          ...
          <div className="flex flex-col flex-1 w-full relative">
            <div className={`w-full border rounded-xl px-4 py-2 relative bg-white transition-all group ${error ? 'border-[#db2e38]' : 'border-slate-300 focus-within:border-slate-900 focus-within:ring-1 focus-within:ring-slate-900'}`}>
              ...
              <div className="flex items-center mt-0.5">
                ...
                />
                {tempEmail.length > 0 && (
                  <button onClick={() => { setTempEmail(''); setError(''); }} className="bg-slate-200 text-slate-500 rounded-full p-0.5 ml-2 mt-1 shrink-0">
                    <X size={14} />
                  </button>
                )}
              </div>
            ...
            {error && (
                <span className="text-[#db2e38] text-[12px] font-medium mt-1.5 ml-1">{error}</span>
            )}
