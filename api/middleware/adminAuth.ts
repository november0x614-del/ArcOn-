...
export const authenticateAdmin = (req: Request, res: Response, next: NextFunction) => {
  ...
  if (!process.env.VITE_ADMIN_SECRET) {
    console.error("VITE_ADMIN_SECRET is not configured in the environment variables.");
    return res.status(500).json({ error: "Server misconfiguration" });
  }

  if (!adminSecret || adminSecret !== process.env.VITE_ADMIN_SECRET) {
