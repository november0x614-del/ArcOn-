...
export async function upgradeWallet(walletId: string) {
  ...
  console.log(`[Admin] Manual upgrade requested for wallet ${walletId} (Method pending SDK verification)`);
...
export async function decideApproval(txId: string, decision: "approve" | "reject") {
  const supabase = getSupabaseAdmin();
  ...
  if (fetchError || !tx) throw new Error("Transaction not found or already processed");
  if (tx.status !== "pending_approval") throw new Error("Transaction is not in pending_approval state");

  if (decision === "reject") {
    const { error: updateError } = await supabase
      .from("transactions")
      .update({ status: "failed", description: `[Rejected] ${tx.description}` })
      .eq("id", txId);
