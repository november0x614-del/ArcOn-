...
const MEMO_TOPIC =
  "0x" + ethers.id("MemoSent(address,address,uint256,bytes)").slice(2);
...
export async function startArcMonitor(supabase: any) {
  console.log(
    `[ArcMonitor] Initializing for Arc Network (Deterministic Finality Mode)...`,
  );
  ...
  try {
    ...
    console.warn(
      `[ArcMonitor] WebSocket failed, falling back to sub-second polling: ${RPC_URL}`,
    );
  ...
  const runMonitor = async () => {
    try {
      ...
      if (error.message?.includes("limit reached") || error.code === -32008) {
        ...
        console.warn(
          `[ArcMonitor] Rate limit detected. Backing off for ${pollInterval}ms`,
        );
      }
    }
...
async function processTransactionGroup(
  ...
  console.log(
    `[ArcMonitor] Processing Arc Native Transfer. To: ${to}, Amount: ${amount}, Memo: ${memoText}`,
  );
  ...
  try {
    await ArcNativeService.sweepFunds(
      wallet.native_private_key,
      internalAmount,
    );
    await supabase
      .from("transactions")
