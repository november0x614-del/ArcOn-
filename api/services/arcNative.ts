...
export class ArcNativeService {
   ...
   */
  static getDepositAddress(userIndex: number): {
    address: string;
    privateKey: string;
  } {
    if (!this.MASTER_MNEMONIC) {
      throw new Error(
        "ARC_MASTER_MNEMONIC is not configured in environment variables",
      );
    }
  ...
  static async sweepFunds(
    ...
    console.log(
      `[ArcNative] Sweeping ${internalAmount} units (internal) to hot wallet ${hotWallet}`,
    );
    ...
    if (!receipt)
      throw new Error("Sweep transaction failed to provide receipt");
